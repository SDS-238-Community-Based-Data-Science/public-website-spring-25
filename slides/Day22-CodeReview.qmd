...
my_plot <- solidago_docket_grants |> 
  mutate(major_unique_funders = as.numeric(major_unique_funders)) |> 
  ggplot(aes(x = major_unique_funders)) + 
...
![](img/solidago_unique_funders.png)
------------------------------------------------------------------------
...
- Each team of 2-3 should review at least two visualizations. Select 2 from the print-outs on your table.
  1. Complete the code review checklist for each visualization and discuss its relevance and effectiveness
  2. Leave in-line comments on the visualization at GitHub (you have already been assigned as a reviewer on the appropriate pull request)
  3. Click the "Review Changes" button, leave a collegial comment, and then "Finish Review"
